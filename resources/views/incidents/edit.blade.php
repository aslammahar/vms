        ...
        <div class="card-body">
            <form action="{{ route('incident.update', $incident->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="vehicle_id" name="vehicle_id" required>
                                ...
                                @foreach($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" {{ $incident->vehicle_id == $vehicle->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="driver_id" id="driver_id" class="form-control" required>
                                ...
                                @foreach($drivers as $driver)
                                    <option value="{{ $driver->id }}" {{ $incident->driver_id == $driver->id ? 'selected' : '' }}>
                                        {{ $driver->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" name="incident_date" id="incident_date" class="form-control" value="{{ old('incident_date', $incident->incident_date) }}" required>
                            @error('incident_date')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" required>{{ old('description', $incident->description) }}</textarea>
                    @error('description')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="severity" id="severity" class="form-control" required>
                                ...
                                <option value="Severe" {{ $incident->severity == 'Severe' ? 'selected' : '' }}>Severe</option>
                            </select>
                            @error('severity')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
